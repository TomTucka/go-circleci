...
// in the post body.
// Returns the new build information
func (c *Client) BuildOpts(vcsType, account, repo, branch string, opts map[string]interface{}) (*Build, error) {
	build := &Build{}

	err := c.request("POST", fmt.Sprintf("project/%s/%s/%s/tree/%s", vcsType, account, repo, branch), build, nil, opts)
	if err != nil {
		return nil, err
	}
